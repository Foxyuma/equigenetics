...
export default function Shop() {
  ...
  const buyMutation = useMutation({
    mutationFn: async ({ item, currency }) => {
      ...
      if (currency === 'genesis') {
        const balance = currentUser?.genesis_balance ?? 200000;
        if (balance < item.price) throw new Error('Insufficient Genesis');
        await base44.auth.updateMe({ genesis_balance: balance - item.price });
      } else if (currency === 'both') {
        const genBalance = currentUser?.genesis_balance ?? 200000;
        const credBalance = currentUser?.credits_balance ?? 200;
        if (genBalance < item.price) throw new Error('Insufficient Genesis');
        if (credBalance < creditsForRare) throw new Error(`Insufficient credits (${creditsForRare} ✦ required)`);
        await base44.auth.updateMe({
          genesis_balance: genBalance - item.price,
          credits_balance: credBalance - creditsForRare,
        });
      } else {
        const balance = currentUser?.credits_balance ?? 200;
        if (balance < item.price) throw new Error('Insufficient credits');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-stone-800 tracking-tight">Shop</h1>
          <p className="text-stone-500 mt-1">Buy consumables for your horses</p>
        </div>
        <Badge className="bg-amber-100 text-amber-700 border-0 text-lg px-4 py-2">
          <Package className="w-4 h-4 mr-2" />
          {inventory.reduce((sum, inv) => sum + inv.quantity, 0)} items
        </Badge>
      </div>
        ...
        <TabsContent value={filterType} className="mt-6">
          {isLoading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-stone-400">No items in this category</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtered.map(item => (
